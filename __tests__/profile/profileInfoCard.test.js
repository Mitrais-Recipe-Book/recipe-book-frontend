...
describe ("ProfileCard", () => {
    ...
    it ("should render the profile info card with data from props", () => {
        ...
        expect(screen.getByText(data.response.totalRecipes + " recipes")).toBeInTheDocument();
        expect(screen.getByText(data.response.recipeLikes + " likes")).toBeInTheDocument();
        expect(screen.getByText(data.response.followers + " followers")).toBeInTheDocument();
    })
    ...
    it ("should render follow/unfollow button", () => {
        ...
        render(<ProfileInfo userData={data} dataQueryParam={dataQueryParam} session={session} creatorId={6}/>);
